---
title: 'Generics'
description: 'Learn about generics (parameterized types) introduced in Go 1.18'
---
...
Let's start building our generic `sum` function. Here, we will use `T` as our type parameter with an empty `interface{}` as our constraint:
...
Luckily, Go 1.18 comes with **type inference** which helps us write code that calls generic functions without explicit types:
...
<Warning>
Now if we run this, we will get an error that operator `+` is not defined in the constraint.
...
Let's define our own custom constraint using an interface. Our interface should define a type set containing `int`, `float`, and `string`:
...
First, we need to install the `constraints` package:
...
So, when should we use generics? We can take the following use cases as examples:
...
<Warning>
While generics are a great addition to the language, they should be used sparingly.
